...
const SidebarTenant = ({ children }) => {
  ...
  useEffect(() => {
    if (userProfile.roles !== "admin_tenant") {
      navigate("/");
    }
  }, [userProfile]);
  ...
  const menuItems = [
    { name: "Order", icon: <IoFastFoodOutline />, path: "/ordertenant" },
    { name: "Kelola Stok", icon: <FaBox />, path: "/stoktenant" },
    { name: "Settings & Utility", icon: <IoMdSettings />, path: "/settingstenant" },
    {
      name: "Logout",
      icon: <SlLogout />,
      action: handleLogout
    ...
    setOrders((prev) =>
      prev.map((order) =>
        order.id === orderId
          ? {
            ...order,
            status: ["WAITING", "SUCCESS", "TAKE AWAY"][
              (["WAITING", "SUCCESS", "TAKE AWAY"].indexOf(order.status) +
                1) %
              3
            ],
          }
    ...
    <div className="flex h-screen bg-gradient-to-br from-blue-50 via-gray-100 to-blue-100 overflow-hidden">
      ...
      <div
        className={`
          ...
          } // kalau close → hilang di mobile, 20px di desktop
                        `}
      >
        {/* Logo Section */}
        ...
        <nav className="flex-1 py-4 space-y-1">
          {menuItems.map((item, index) =>
            item.action ? (
              // 🔴 kalau ada action, jangan pakai <Link>
              <button
                key={index}
                onClick={item.action}
            ...
            <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-gray-300 flex items-center justify-center">
              ...
              >
                <path
                  ...
                  strokeWidth="1.5"
                  d="M5.121 17.804A9.969 9.969 0 0112 15c2.21 0 4.236.714 5.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0z"
                />
              </svg>
            </div>
          </div>
