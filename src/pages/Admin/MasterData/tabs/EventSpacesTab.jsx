...
import {
  Table, Button, Modal, Input, Typography, Row, Col, Card, Space, message, Popconfirm, Tooltip, InputNumber, Select, Tag,
  Upload, Image // Perubahan: Import Upload dan Image
} from "antd";
import {
  PlusOutlined, EditOutlined, DeleteOutlined, SearchOutlined, 
  UploadOutlined // Perubahan: Import ikon untuk tombol upload
...
const { Option } = Select;
// Perubahan: Definisikan base URL untuk menampilkan gambar
const UPLOAD_URL = `${import.meta.env.VITE_BASE_URL}/static/`; // Sesuaikan dengan path folder statis di Flask
...
const EventSpacesTab = () => {
  ...
  const [fileList, setFileList] = useState([]); // Perubahan: State baru untuk mengelola file gambar
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (res.status === 200 && res.data.message === "OK") {
        const eventSpaces = res.data.datas.map((item, index) => ({
  ...
  const columns = [
    // Perubahan: Tambah kolom untuk menampilkan gambar
    {
      title: "Gambar",
    ...
    { title: "Kapasitas", dataIndex: "kapasitas", key: "kapasitas", render: (text) => text ? `${text} orang` : '-' },
    {
      ...
      render: (status) => (
        <Tag color={status === 'Tersedia' ? 'green' : 'red'}>{status}</Tag>
      ),
    },
    {
      title: "Actions",
  ...
  // Perubahan: Handler untuk saat file di komponen Upload berubah
  const handleFileChange = ({ fileList: newFileList }) => {
    setFileList(newFileList);
  };

  // Perubahan: Logika handleSave diubah total untuk mengirim FormData
  const handleSave = async () => {
    ...
    if (fileList.length > 0 && fileList[0].originFileObj) {
      ...
      // Jika sedang edit dan tidak ada gambar baru, kirim nama file lama untuk referensi di backend
  ...
  // Perubahan: handleEdit diupdate untuk menampilkan gambar yang sudah ada
  const handleEdit = (record) => {
    setEditingEventSpace(record);
    setFormData(record);
    if (record.gambar_ruangan) {
      // Buat objek file agar komponen Upload bisa menampilkannya
      setFileList([{
        uid: '-1',
  ...
  // Perubahan: handleCancel diupdate untuk mereset fileList
  const handleCancel = () => {
    setOpen(false);
    setFormData({});
    ...
    <div style={{ padding: "24px" }}>
      ...
      >
        <div style={{ marginTop: '24px' }}>
          <Text strong>Nama Event Space <span style={{ color: "red" }}>*</span></Text>
          <Input placeholder="Cth: Aula Serbaguna" value={formData.nama_event_space || ""} onChange={(e) => handleChange("nama_event_space", e.target.value)} style={{ marginTop: '8px' }} />
        </div>
        <div style={{ marginTop: '16px' }}>
          <Text strong>Deskripsi</Text>
          <TextArea rows={3} placeholder="Masukkan deskripsi singkat" value={formData.deskripsi_event_space || ""} onChange={(e) => handleChange("deskripsi_event_space", e.target.value)} style={{ marginTop: '8px' }} />
        </div>
        <Row gutter={16}>
          <Col span={12}>
            <div style={{ marginTop: '16px' }}>
              <Text strong>Harga Paket (Rp) <span style={{ color: "red" }}>*</span></Text>
              <InputNumber style={{ width: '100%', marginTop: '8px' }} placeholder="Cth: 1500000" value={formData.harga_paket} onChange={(value) => handleChange("harga_paket", value)} formatter={(value) => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')} parser={(value) => value.replace(/\$\s?|(,*)/g, '')} />
            </div>
          </Col>
          <Col span={12}>
            <div style={{ marginTop: '16px' }}>
              <Text strong>Kapasitas (Orang)</Text>
              <InputNumber style={{ width: '100%', marginTop: '8px' }} placeholder="Cth: 50" value={formData.kapasitas} onChange={(value) => handleChange("kapasitas", value)} />
            </div>
          </Col>
        </Row>
        
        {/* Perubahan: Tambah komponen Upload di dalam Modal */}
        <div style={{ marginTop: '16px' }}>
          ...
          <Upload
            ...
            beforeUpload={() => false} // Mencegah upload otomatis ke action bawaan
            maxCount={1}
            style={{ marginTop: '8px' }}
          >
        ...
        <div style={{ marginTop: '16px' }}>
          ...
          <Select style={{ width: '100%', marginTop: '8px' }} placeholder="Pilih status" value={formData.status_ketersediaan} onChange={(value) => handleChange("status_ketersediaan", value)}>
        ...
        <div style={{ marginTop: '16px' }}>
          ...
          <TextArea rows={3} placeholder="Cth: free wifi, proyektor, sound system" value={formData.fitur_ruangan || ""} onChange={(e) => handleChange("fitur_ruangan", e.target.value)} style={{ marginTop: '8px' }} />
        </div>
      </Modal>
    </div>
  );
